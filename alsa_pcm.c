...
int pcm_write(struct pcm *pcm, void *data, unsigned count)
{
    ...
    for (;;) {
        ...
        if (ioctl(pcm->fd, SNDRV_PCM_IOCTL_WRITEI_FRAMES, &x)) {
            ...
            else if ((pcm->flags & PCM_RATE_MASK) == PCM_16000HZ)
                usleep_time = frames * 1000 * 1000 / 16000;
            else
...
__open_again:
    ...
    } else if ((flags & PCM_RATE_MASK) == PCM_16000HZ) {
        ALOGD("set audio rate 16KHz");
        param_set_int(&params, SNDRV_PCM_HW_PARAM_RATE, 16000);
    } else
